...
//following code will load variable products when a server request is made for products.js. v
...
app.post('/purchase', function(request, response) {
    const selectedItem = request.body.selectedItem;
    const quantity = parseInt(request.body.quantity);

    // Validate data
    if (!selectedItem || isNaN(quantity) || quantity <= 0) {
        return response.status(400).send('Invalid data. Please enter valid quantity.');
    }

    // Implement further logic for processing the purchase

    // Send a success response
    response.send('Purchase successful!');
});
